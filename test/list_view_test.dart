...
void main() {
  group('DayView Widget Tests', () {
    ...
    testWidgets('should display time slots when room is selected', (tester) async {
      final testLocation = createTestLocation();
      final testRoom = testLocation.rooms.first;
      ...
      // Should display time slot cards
      expect(find.text('Available - Click to book'), findsWidgets);
    });

    testWidgets('should display bookings in time slots', (tester) async {
      final testLocation = createTestLocation();
      final testRoom = testLocation.rooms.first;
      final testBookings = createTestBookings();
      ...
      // Should display booking courses
    ...
    testWidgets('should show booking details when booked slot is tapped', (tester) async {
      final testLocation = createTestLocation();
      final testRoom = testLocation.rooms.first;
      final testBookings = createTestBookings();
    ...
    testWidgets('should handle different time slot intervals', (tester) async {
      final testLocation = createTestLocation();
      final testRoom = testLocation.rooms.first;
      ...
      // Should display time slots with 15-minute intervals
    ...
    testWidgets('should validate booking form input', (tester) async {
      final testLocation = createTestLocation();
      final testRoom = testLocation.rooms.first;
      ...
      // Try to book without entering course name
      await tester.tap(find.text('Book'));
      await tester.pumpAndSettle();

      // Should show validation error
      expect(find.textContaining('Course name is required'), findsOneWidget);
    });

    testWidgets('should display user bookings with delete option', (tester) async {
      final testLocation = createTestLocation();
      final testRoom = testLocation.rooms.first;
      final testBookings = createTestBookings();
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            selectedLocationProvider.overrideWith((ref) => testLocation),
            selectedRoomProvider.overrideWith((ref) => testRoom),
            bookingsListProvider.overrideWith((ref) => testBookings), // User bookings
            bookingsProvider.overrideWith((ref) => []), // Mock bookings
          ],
          child: MaterialApp(
            home: Scaffold(body: const DayView()),
          ),
      ...
      // Tap on a user booking
      await tester.tap(find.text('Flutter Workshop'));
      await tester.pumpAndSettle();

      // Should show delete option for user bookings
      expect(find.text('Your booking - You can delete this'), findsOneWidget);
